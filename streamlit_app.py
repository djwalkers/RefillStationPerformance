...
if active_tab == "Hourly Dashboard":
    ...
    dashboard_tab(data.copy(), "hourly", time_filters=True, week_filter=False, month_filter=False)
    ...
    # ---- TOP PICKER PER DAY (ALL HOURS, CALENDAR DAY) ----
    st.subheader("Top Picker Per Day (Whole Day)")
    top_per_day = (
        data.groupby(["Date", "Users", "Station Type"], as_index=False)
        ["Carts Counted Per Hour"].sum()
    )
    idx = (
    ...
    shift_data = (
        data.groupby(["Date", "Shift", "Users", "Station Type"], as_index=False)
        ["Carts Counted Per Hour"].sum()
    )
